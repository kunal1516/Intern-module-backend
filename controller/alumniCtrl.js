...
const deleteAlumni = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res.status(201).json(delAlumni);
  } catch (error) {
    throw new Error(error);
  }
});
...
const updateAlumni = asyncHandler(async (req, res) => {
  const { id } = req.params;
  try {
    const updateAlu = await Alumni.findByIdAndUpdate(
      id,
      {
...
const handleRefreshToken = asyncHandler(async (req, res) => {
  try {
    ...
    if (!refreshToken) {
      return res
        .status(400)
        .json({ error: "No refreshToken found in cookies." });
    }
    ...
    if (!alumni) {
      return res
        .status(400)
        .json({ error: "Refresh token not matched with any user." });
    }

    if (String(alumni._id) !== decoded.id) {
      return res
        .status(400)
        .json({ error: "Invalid refresh token: Alumni ID mismatch." });
    }
